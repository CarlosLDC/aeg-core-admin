...
export function ClientModificationRequestsManager() {
  const toast = useToast();
  const [rows, setRows] = useState<ClientModificationRequestListItemResponse[]>([]);
  ...
  const load = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      const data = [
        ...(await fetchClientModificationRequests("PENDING")),
        ...(await fetchClientModificationRequests("APPROVED")),
        ...(await fetchClientModificationRequests("REJECTED")),
      ];
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (!q) return true;
      return `${row.id} ${row.clientName} ${row.requestedByName} ${row.actionType}`
        .toLowerCase()
        .includes(q);
    });
