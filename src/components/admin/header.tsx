...
export function Header({
  ...
  return (
    <header
      className={cn(
        "fixed top-0 right-0 z-30 box-border flex h-[var(--admin-header-height)] items-center border-b border-border bg-background/95 backdrop-blur-md transition-[left] duration-300 ease-out supports-[backdrop-filter]:bg-background/80",
        "left-0",
        sidebarCollapsed ? "lg:left-[72px]" : "lg:left-64",
      )}
    >
      <div className="flex h-full w-full min-w-0 items-center gap-2 px-3 sm:gap-4 sm:px-6 lg:px-8">
        <button
          type="button"
          onClick={onMenuClick}
